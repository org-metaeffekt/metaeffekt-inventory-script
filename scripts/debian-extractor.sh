...
dpkg-query --show --showformat="{$packagetag"',"name":"${Package}","version":"${Version}","release":null,"arch":"${Architecture}","group":null,"license":null,"sourcerpm":null,"packager":"${Maintainer}","vendor":null,"url":"${Homepage}"\}\n' > "$Metaeffekt_Inv_Basedir/inventory-full.tmp.json"
